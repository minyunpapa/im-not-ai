...
- **C-13** 줄바꿈이 길이 단위 → **의미 단위로 재배치**: 문장 중간 개행 제거 + 화제·논점 전환 지점에 빈 줄. 나열 3개 미만은 문장으로 잇는다(토막내면 C-1·C-2 로 역주행)
...
6. **줄바꿈 = 의미 단위**: 한 문장이 중간에서 끊기지 않았는가, 화제가 바뀌는 자리에 빈 줄이 있는가 (C-13)
7. **인공 표현 자제 (빼기 전용)**: 원문에 없던 비유·수사·상투구("기록적인 성과·~로 평가된다" 등)를 윤문 과정에서 새로 심지 않았는가. 살아있는 구어(부가설명 대시·짧은 감탄·반문)는 보존
