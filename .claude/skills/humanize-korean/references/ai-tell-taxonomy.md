...
### C-13. 길이 단위 줄바꿈 (맥락 무시 개행) [S2] · v2.4 신규 · 사용자 지시 2026-08-18
- 패턴: 줄바꿈이 **의미 단위가 아니라 길이 단위**로 들어간다. 한 호흡으로 읽어야 할 문장이 중간에서 끊기고, 반대로 화제가 바뀌는 자리는 붙어 있다. 원인은 "적당히 길어지면 줄을 바꾼다"는 기계적 습관이다. 사람이 쓴 글은 **하나의 생각이 끝나는 자리**에서 끊는다.
- 예(나쁨): "이 수치는 코호트 3만 명에서 나온 것이고\n보정 변수는 나이와 성별이다. 다만 흡연은\n빠져 있어서 해석에 주의가 필요하다"
  → 한 덩어리(수치의 출처)가 두 줄로 잘리고, 화제 전환("다만 흡연은")은 줄 중간에 묻혔다.
- 예(좋음): "이 수치는 코호트 3만 명에서 나온 것이고 보정 변수는 나이와 성별이다.\n\n다만 흡연이 빠져 있어서 해석에 주의가 필요하다."
- 처방: ① 한 문장을 중간에서 끊지 않는다 ② 화제·논점이 바뀌면 반드시 끊는다(빈 줄) ③ 근거→판단, 문제→처방, 사실→해석 같은 **전환 지점**을 줄바꿈 자리로 삼는다 ④ 나열이 3개를 넘고 각 항이 독립적일 때만 항목당 줄바꿈. 2개 이하는 문장으로 잇는다.
- ⚠️ 역방향 금지: 줄바꿈을 **늘리는 것**이 목적이 아니다. 의미 단위가 길면 긴 줄로 두는 게 맞다. 짧게 토막내면 C-1(기계적 병렬)·C-2(불릿 남발)로 되돌아간다.
- _quick: true · quick_pattern: 문장 중간 개행 / 화제 전환에 빈 줄 없음 · quick_fix: 문장 중간 개행 제거 후, 화제 전환 지점에 빈 줄 삽입_
